...
if (window.matchMedia("(min-width: 1281px)").matches) {
  $("#slidewrap ul .s1 a> img").css({
    transform: "scale(2.45, 2.25)",
    "z-index": 1,
  });
...
document.querySelector("#section1 ul .s1> img").style.transform =
  "translateY(0)";
document.querySelector("#section1 ul .s1> img").style.opacity = 1;
document.querySelector("#section1 ul .s1> span").style.transform =
  "translateY(0)";
...
function slide() {
  if (window.matchMedia("(min-width: 769px)").matches) {
    if (page == 1) {
      $("#slidewrap ul")
        .stop()
        .animate({ marginLeft: "-14%" }, 2000, function () {
          $("#slidewrap ul li:nth-of-type(2) a> img").css({
            transform: "scale(2.45, 2.25)",
            transition: "2s",
          });
        });
      ...
      if (
        ...
        $("#slidewrap ul")
          .stop()
          .animate({ marginLeft: 0 }, 2000, function () {
            $("#slidewrap ul li:nth-of-type(2) a> img").css({
              transform: "scale(2.15, 2.55)",
              transition: "2s",
            });
      ...
      $("#slidewrap ul li:nth-of-type(1) a> img").css({
        transform: "scale(1)",
        "z-index": 0,
      });
      document.querySelector("#section1 ul .s1> img").style.transform =
      ...
      $("#slidewrap ul")
        .stop()
        .animate({ marginLeft: "-57.7%" }, 2000, function () {
      ...
      if (
        ...
        $("#slidewrap ul")
          .stop()
          .animate({ marginLeft: "-53.08%" }, 2000, function () {
      ...
      if (page > 16) {
        ...
        function fslide() {
          ...
          if (
            window.matchMedia("(min-width:1281px) and (max-width:1400px)")
              .matches
          ) {
            $("#footerwrap").animate(
              { marginTop: "-160px" },
  ...
  if (window.matchMedia("(max-width: 768px)").matches) {
    function sl2() {
      $("#slidewrap ul")
        .stop()
        .animate({ marginLeft: "-111%" }, 2000, function () {
          logo[page - 1].style.transform = "translate(0,0)";
          logo[page - 1].style.opacity = 1;
    ...
    function fslide() {
      if (
        window.matchMedia("(min-width:431px) and (max-width:768px)").matches
      ) {
